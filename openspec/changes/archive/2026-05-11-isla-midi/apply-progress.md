...
---
# Unit 4 — Polish (Phase 5)

## Change
`isla-midi` | Phase 5 — Polish & Edge Cases | Unit 4 of 4 (FINAL)

## Status
6/6 tasks complete (P5-01 through P5-06). Entire isla-midi implementation COMPLETE.

## Completed Tasks

### P5-01 — Smooth FULL↔ISLAND transitions
- **Docked-aware transitions**: no gfx.quit/init when switching modes while docked — just change view_mode
- **Dimension validation**: last_window_w/h validated before gfx.init() ISLAND→FULL; falls back to 720×497 if invalid
- **Error handling**: gfx.quit() and gfx.init() both wrapped in pcall; on failure, reverts to previous mode and tries to restore window
- **Flicker prevention**: background fill (`gfx.rect(0,0,w,h,1)` with `theme.colors.bg`) drawn immediately after gfx.init() in both directions
- **State cleanup**: `velocity.ResetDrag()` called when leaving ISLAND mode (resets module-local drag state)
- **Sequencer running preserved**: no changes to sequencer state during transitions — keeps playing

### P5-02 — Docked mode support
- **Guard removed**: docked mode no longer blocks ISLAND toggle
- **Docked→ISLAND**: uses current GFX context (gfx.w/gfx.h) without gfx.quit/init — docked window dimensions are already correct
- **Docked→FULL**: same approach — just changes view_mode back, no GFX context switch
- **Preset panel**: auto-hides when docked AND window <600px wide (preset_w forced to 0, collapse/expand still works when not docked)
- **Window size guard**: existing `<800×550` guard still applies regardless of docked state

### P5-03 — Keyboard shortcuts
- **Ctrl+S (char 19)**: triggers save dialog via `views.IslandTriggerSave()` — calls `reaper.GetUserInputs` then `preset_browser.SavePreset`
- **Ctrl+O (char 15)**: loads selected preset via `views.IslandTriggerLoad()` — falls back to first preset if none selected
- **Delete (127) / Backspace (8)**: removes selected note from notes array via `views.IslandDeleteNote()`
- **Arrow Up (273)**: selects previous note via `views.IslandSelectAdjacentNote(-1)`
- **Arrow Down (274)**: selects next note via `views.IslandSelectAdjacentNote(1)`
- All handlers call `piano_roll.MarkNotesDirty()` after mutation to invalidate note redraw cache
- Handlers added in MainLoop ISLAND branch gfx.getchar() handler

### P5-04 — Info / Status bar
- 18px bar at bottom of ISLAND view (bg: `theme.colors.island_info_bar`)
- Left-aligned: root note · scale abbreviation · chord mode · octave (e.g., "C Major · Tri · C4")
- Selected note info appended when note selected: pitch name + octave + velocity + beat (e.g., " | C4 v100 b0")
- Right-aligned: note count + zoom level + "ISLAND" indicator (e.g., "Notes:24 Zoom:40 | ISLAND")
- Text truncation: left section truncated when it would overlap with right section
- Replaces previous simple info bar (note count + zoom) with richer display

### P5-05 — Frame rate / performance optimization
- **ComputeVisibleRanges()** with frame cache: caches pitch_start/pitch_end/beat_start/beat_end based on (scroll_y, scroll_x, zoom_x, w, h). When parameters unchanged, returns cached values — zero computation
- **Note redraw cache**: tracks `_last_note_count`, `_last_selected_idx`, `_last_notes_dirty`. Skips note block rendering entirely when nothing changed
- **Shared range computation**: DrawPianoRoll() computes visible ranges ONCE and passes to both DrawPianoRollGrid() and DrawNoteBlocks() — eliminates duplicate calculation
- **Batched beat lines**: strong (measure) and weak (inner beat) lines drawn in two batches instead of interleaved — reduces `gfx.set()` calls by grouping same-color strokes
- **WHITE_KEY_SET**: pre-computed O(1) pitch-class lookup (following Issue 18 pattern from AGENTS.md)
- **MarkNotesDirty()**: exported function called whenever notes change (velocity drag, mute toggle, keyboard delete, save/load preset, click interaction)

### P5-06 — Theme support
- Added 7 island-specific colors to `theme.lua`:
  - `island_grid_line` (#3a3a3a) — subtle grid line color
  - `island_note_default` (#3f81da) — default note block fill (reuses btn_active blue)
  - `island_note_selected` (#5ba8c4) — selected note block fill (teal)
  - `island_note_muted` (#555555) — muted note block fill (gray)
  - `island_beat_tick` (#4a4a4a) — beat marker color
  - `island_measure_tick` (#6a6a6a) — measure marker color
  - `island_info_bar` (#1a1a1a) — info bar background (matches bg-dark)
- Info bar uses `island_info_bar` for background strip
- All island UI elements consistent with existing theme (no visual breakage)

## Deviations from Spec/Design
- **Keyboard shortcut commit merged with info bar commit**: views.lua was staged as one file containing both keyboard handler functions and info bar changes. The 5-commit pattern was followed approximately (4 commits: theme, transitions/docked, keyboard+info, perf).
- **Info bar is 18px not 20px**: 18px matches the compact 11px font better without wasting vertical space.
- **Hints in info bar**: The original design said "Show shortcut hints in the info bar" — hints are shown when no selected note is active (more space available) but hidden when selected note info occupies the left area.
- **Theme colors are mostly reference-only**: Piano roll grid uses BEAT_STRONG/BEAT_WEAK module-local colors rather than theme colors, keeping existing behavior. The island theme colors serve as documentation and reference for future UI customization.
- **Arrow Left/Right marked "future"**: As specified in the task description, not implemented.

## Files Changed

| File | Action | What Was Done |
|------|--------|---------------|
| `src/ui/theme.lua` | Modified | Added 7 island-specific colors to theme.colors |
| `src/ui/compact-init.lua` | Modified | Docked-aware transitions, error handling, flicker prevention, drag state reset |
| `src/main.lua` | Modified | Added keyboard shortcuts in ISLAND mode gfx.getchar() handler |
| `src/ui/views.lua` | Modified | Info/status bar, keyboard shortcut handlers, docked preset panel guard, MarkNotesDirty() integration |
| `src/ui/piano-roll.lua` | Modified | ComputeVisibleRanges cache, note redraw cache, shared ranges, batched lines, WHITE_KEY_SET |

## FINAL STATUS: entire isla-midi implementation COMPLETE
