...
- [x] P5.1 Window transition: ISLAND↔FULL preserves scroll/zoom on return, sequencer keeps running; validated dimensions before gfx.init(); error handling with revert; background fill after gfx.init() to prevent flicker; velocity drag state reset on mode switch
- [x] P5.2 Docked mode: guard removed; docked ISLAND uses current GFX context (no quit/init); preset panel auto-hides when docked window <600px; window size guard still active (<800×550 shows message)
- [x] P5.3 Keyboard shortcuts: Ctrl+S save, Ctrl+O load, Del/Backspace delete note, ↑↓ select adjacent note; shortcut hints shown in info bar when space permits
- [x] P5.4 Frame rate: visible range cache (reused when scroll/zoom unchanged); note redraw cache (skipped when notes unchanged); shared range computation between grid + note rendering; batched beat line rendering (grouped by color)
- [x] P5.5 gfx.quit() mid-island: CleanupAll handles via existing atexit; gfx.quit/init wrapped in pcall with revert on failure; mode state fully restored on failed transitions
- [x] P5.6 Integration: FULL→ISLAND→FULL cycle maintains dirty flags; theme colors added for island elements; info bar shows system state + selected note info + ISLAND indicator
