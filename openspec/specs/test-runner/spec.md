...
|----|------------|-----------|
| P0 | `run.lua` MUST assign `_G.reaper` and `_G.gfx` to their mock tables BEFORE extending `package.path`. Any require of `src/` modules MUST have globals in place. | GIVEN run.lua loads mock globals WHEN extending package.path THEN `require("core.midi")` succeeds without "attempt to index a nil value (global 'reaper')" error. GIVEN test file A made calls WHEN transitioning to file B THEN `reset_all_calls()` zeroes all mock call counts |
| P1 | `run.lua` MUST derive `src/` path from own location via `debug.getinfo(1,'S')`, normalize backslashes to forward slashes, prepend to `package.path`. This MUST happen AFTER global installation. | GIVEN run.lua at `D:\project\tests\run.lua` WHEN ready to extend `package.path` (after globals installed) THEN `package.path` starts with `D:/project/src/?.lua;`. GIVEN Windows path with backslashes WHEN normalized THEN package.path uses forward slashes |
